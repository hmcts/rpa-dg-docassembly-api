...
locals {
  ...
  s2s_base_uri = "http://${var.s2s_base_uri}-${local.local_env}.service.${local.local_ase}.internal"
}
...
module "app" {
  ...
  app_settings = {
    ...
    S2S_BASE_URI = "http://${var.s2s_base_uri}-${local.local_env}.service.core-compute-${local.local_env}.internal"
